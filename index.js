...
const userStart = [230, 10] //where you start no matter what
...
class Block{
    ...
    } //where each point of our block location is at any given time
...
function moveUser(e) {
    switch(e.key) {
        ...
        case 'ArrowRight':
            ...
            break;
    }//listen out for keys - either left or right
}
...
timerId = setInterval(moveBall, 30) //can make the game harder by decreasing this number
...
function checkForCollisions(){
    ...
    for(let i = 0; i < blocks.length; i++){
        if(
            ...
            //will still be there, so need to get rid of block
            blocks.splice(i, 1) //to remove the one item by using splice
            changeDirection()
            score++
